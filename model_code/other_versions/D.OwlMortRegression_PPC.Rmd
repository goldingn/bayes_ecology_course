...
model
{
  ...
  for (i in 1:26)
  {
    ...
    # simulate a new value at this location 
    logitMortalitySim[i] ~ dnorm(lp[i], tau)
...
```{r params}
params <- c("a", "b", "c", "tau", "logitMortalitySim")
```
...
We can run a visual posterior predictive check on this model using the bayesplot package
```{r ppc}
library(coda)
draws <- as.matrix(fit$samples)
sims <- draws[1:100, 5:30]

library(bayesplot)
ppc_dens_overlay(data$logitMortality, sims)
```