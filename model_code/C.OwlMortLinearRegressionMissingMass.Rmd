...
model
{
  ...
  meanMass ~ dnorm(0, 1.0E-6)
  tauMass ~ dgamma(0.001, 0.001)

  # for each of the 26 raptors
  # calculate the expected mortality
  for (i in 1:26)
  {
    Mass[i] ~ dlnorm(meanMass, tauMass)
...
```{r data}
data <- list(
  Mass = c(NA, 0.28, 0.64, 0.14, 1.2, 0.58, 0.9, 0.3, 1.6, 0.885, 0.53, 
...
```{r params}
params <- c("a", "b", "c", "tau", "MissingMass")
```
